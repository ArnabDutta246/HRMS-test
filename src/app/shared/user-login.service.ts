...
export class UserLoginService {
  ...
  getOrgAndPlanData(){
    ...
    let orgData = this.afs.collection(this.allCol.subscribers).doc(getSession.subscriberId).ref;
    ...
    async getPlanData(check?:any){
      console.log("the curr url",check);

     await this.getOrgAndPlanData().then((data:any)=>{
  ...
  storeInLocalStorage(subStatus: boolean) {
    let data = { ...this.currentuser, subscriptionExpire: subStatus,admin:{...this.subsData}};
